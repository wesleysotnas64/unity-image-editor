...
namespace Scripts.Effects
{
    static class NonLinearEffects
    {
        ...
        public static Texture2D FourierTransform(Texture2D inputTexture)
        {
            ...
            Color[] inputData = inputTexture.GetPixels();

            for (int u = 0; u < width; u++)
            {
                for (int v = 0; v < height; v++)
                {
                    Color transformedColor = ComputeFourierTransform(inputData, width, height, u, v);
                    outputData[u + v * width] = transformedColor;
                }
            }
        ...
        private static Color ComputeFourierTransform(Color[] inputData, int width, int height, int u, int v)
        {
            ...
            for (int x = 0; x < width; x++)
            {
                ...
                {
                    Color pixelColor = inputData[x + y * width];
                    float theta = 2 * Mathf.PI * (u * x / (float)width + v * y / (float)height);
                    result += pixelColor * new Color(Mathf.Cos(theta), Mathf.Sin(theta), 0, 1);
                }
            }

            float magnitude = result.r * result.r + result.g * result.g;
            magnitude = Mathf.Clamp01(magnitude);

            result = new Color(magnitude, magnitude, magnitude, 1);
